...
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Content scripts matching URL patterns | [`page-mod`][page-mod-link] API                | [`content-scripts`][content-script-link] key |
| Toolbar buttons                       | [`ui/button/action`][ui-button-action] API | [`browser_action`][browser-action] key   |
| Access privileged APIs                | `require()` function                                                                                             | [`permissions`][permission-link] key         |
...
- [Learn more about `manifest.json`][manifest-link]
...
[background-scripts-link]:https://developer.mozilla.org/Add-ons/WebExtensions/Anatomy_of_a_WebExtension#Background_scripts
...
- [Background scripts for extensions][background-scripts-link]
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Attach scripts to pages matching URL pattern | [`page-mod`][page-mod-link]API     | [`content-scripts`][content-script-link] key |
...
- [Content scripts for WebExtensions][content-link]
...
| ------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Button        | [`ui/button/action`][ui-button-action] | [`browser_action`][browser-action] / [`page_action`][page-action-link] |
| Toggle button | [`ui/button/toggle`][ui-button-toggle] | [`browser_action`][browser-action] / [`page_action`][page-action-link] |
| Toolbar       | [`ui/toolbar`][ui-toolbar]            | None                                                                                                                                                                                                               |
| Sidebar       | [`ui/sidebar`][ui-sidebar]             | `sidebar_action`                                                                                                                                                                                                   |
| Panel         | [`panel`][panel-link]                     | `browser_action` / `page_action` [popup][popup-link]                                                                                                  |
| Context menu  | [`context-menu`](https://developer.mozilla.org/docs/Archive/Add-ons/Add-on_SDK/High-Level_APIs/context-menu) | [`contextMenus`][context-menu-link]                                                                                                               |
...
Panels and [popups][popup-link] are both transient dialogs specified using HTML, CSS, and JavaScript.
...
With the SDK you can define preferences using a `preferences` key in `package.json`. The user can see and change these preferences in the extension's entry in the Add-ons Manager. The extension in turn can listen for changes using the [`simple-prefs`][simple-prefs-link] API.

With WebExtensions, you will have to implement your own UI for presenting settings, and your own code for persisting them for your extension. You do this by writing an HTML file that presents the settings UI, which can include a script for persisting the settings. The script gets access to all the WebExtensions APIs, and it's generally expected that you should use the [`storage`][storage-link] API to persist settings.

You then assign the HTML file's URL to the [`options_ui`][options-ui-link] key in `manifest.json`. Your settings page then appears in the extension's entry in the Add-ons Manager. The options page can also be programmatically opened with an API call to [`browser.runtime.openOptionsPage`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/runtime/openOptionsPage).

Note that WebExtensions does not provide an equivalent of the SDK's [`preferences/service`][preference-link] API, which provides general access to browser settings. However, you can change some browser settings using the [`privacy`][privacy-link] and [`browserSettings`][browser-setting-link] APIs.
...
To retrieve localized strings in extension code, there's a JavaScript API - [`l10n`](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/High-Level_APIs/l10n) in the SDK and [`i18n`][i18n-link] in WebExtensions - that returns a localized string given an ID.
...
The Add-on SDK comes with a command-line tool, [`jpm`](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/Tools/jpm), that you can use for testing and packaging extensions. There's an equivalent tool for WebExtensions, called [`web-ext`][web-ext-link]. web-ext doesn't yet support all the same commands that jpm does, but it has the basics: [`run`](/documentation/develop/web-ext-command-reference#web-ext-run), [`build`](/documentation/develop/web-ext-command-reference#web-ext-build), and [`sign`](/documentation/develop/web-ext-command-reference#web-ext-sign).

It's also now possible to install (and reload) SDK add-ons and extensions built with WebExtension APIs in Firefox from their source directory, without needing to package them as an XPI. See [Temporary Installation in Firefox][temp-firefox-link].

### Learn more

- [web-ext tutorial][web-ext-link]
- [web-ext reference](/documentation/develop/web-ext-command-reference)
- [Temporary installation in Firefox][temp-firefox-link]
...
For some WebExtension APIs, you need to ask permission first, using the [`permissions`][permission-link] `manifest.json` key. In the example below, the extension will need to ask for the "`tabs`" permission if they want access to the tab's URL:
...
| [context-menu](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/High-Level_APIs/context-menu)         | [`contextMenus`][context-menu-link]                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| [indexed-db](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/High-Level_APIs/indexed-db)             | [`window.indexedDB`][indexed-db-link]                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| [l10n](https://developer.mozilla.org/docs/Archive/Add-ons/Add-on_SDK/High-Level_APIs/l10n)                  | [`i18n`][i18n-link]                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| [page-mod][page-mod-link]               | [`content_scripts`][content-script-link]                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| /[panel][panel-link]                       | See [UI elements](#ui-elements) above.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| [request](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/High-Level_APIs/request)                   | [`window.fetch`][window-fetch-link] or [`window.XMLHttpRequest`](https://developer.mozilla.org/docs/Web/API/XMLHttpRequest)                                                                                                                                                                                                                                                                                                                                                                    |
...
| [simple-prefs][simple-prefs-link]         | [`storage`][storage-link] and [`options_ui`][options-ui-link]                                                                                                                                                                                                                                                                                                                           |
| [simple-storage](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/High-Level_APIs/simple-storage)     | [`storage`][storage-link]                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| [net/xhr](https://developer.mozilla.org/docs/Mozilla/Add-ons/SDK/Low-Level_APIs/net_xhr)                                   | [`window.fetch`][window-fetch-link] or [`window.XMLHttpRequest`](https://developer.mozilla.org/docs/Web/API/XMLHttpRequest)                                                                        |
...
| [preferences/service][preference-link]           | Limited support via the [`privacy`][privacy-link] and [`browserSettings`][browser-setting-link] APIs. |
...
| [ui/button/action][ui-button-action]                | [`browser_action`][browser-action] / [`page_action`][page-link]       |
| [ui/button/toggle][ui-button-toggle]                 | [`browser_action`][browser-action] / [`page_action`][page-link]       |
...
| [ui/sidebar][ui-sidebar]                             | [`sidebarAction`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/sidebarAction)                                                                                                                                 |
| [ui/toolbar][ui-toolbar]                            | None                                                                                                                                                                                                                                  |
