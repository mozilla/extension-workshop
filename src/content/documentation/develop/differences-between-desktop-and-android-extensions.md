...
The features of [`action`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action) are also reduced in Firefox for Android. The `manifest.json` key [action](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/manifest.json/action) enables you to define a popup. You then have use of [`action.show()`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action/show) and [`action.hide()`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action/hide) however, once ‘shown’, note that the address bar button is visible in all tabs (unlike the desktop behavior, where the button is shown only for a specified tab.) But you can hide the action using [`action.hide()`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action/hide) on a specific tab (say, for example, you wish to hide your extension's action icon in `about:addons` or `about:memory` tabs) And you can set a listener to [`action.onClicked()`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action/onClicked). [`action.setPopup()`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action/setPopup) and [`action.getPopup()`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action/getPopup) are also available, so you can update the popup or create a popup once the add-on is running.

Also, in both [`pageAction`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/pageAction) and [`action`](https://developer.mozilla.org/docs/Mozilla/Add-ons/WebExtensions/API/action) popup content is opened in a new tab and persists until the user manually closes the tab.
