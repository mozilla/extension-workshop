...
For much of the debugging work, it's useful to be able to view **Console** with **Inspector** or **Debugger**. You do this using the [split console][split-console], press `esc` to activate this mode.
...
::: note
We use the [notify-link-clicks-i18n][i18n-link] extension example to illustrate the debugging features relevant to background scripts. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
To debug background scripts, use the [Toolbox](#developer-tools-toolbox) **Debugger** in the [split console][split-console] view so you can view the **Console** below **Debugger**.
...
For example, the [notify-link-clicks-i18n][i18n-link] example extension logs a message from its background script when it receives a message from one of its content scripts.
...
In the **Debugger** you can set breakpoints, step through code, modify the extension's state, and do [everything else you'd expect to be able to do in a debugger][debugger-link].

![add breakpoint screenshot](/assets/img/documentation/develop/adding_breakpoint.png)

For more information about using the debugger, see the [Debugger][debugger-link] guide.
...
::: note
We use the [favourite-colour](https://github.com/mdn/webextensions-examples/tree/master/favourite-colour) extension example to illustrate the debugging features relevant to options pages. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
To debug options pages, use the [Toolbox](#developer-tools-toolbox) **Debugger** in the [split console][split-console] view so you can view the **Console** below **Debugger**.
...
In the **Debugger** you can set breakpoints, step through code, modify the extension's state, and do [everything else you'd expect to be able to do in a debugger][debugger-link]. Any messages logged by your code display in **Console**.
...
For more information about using **Inspector**, see the [Inspector][page-inspector-link] guide.
...
::: note
We use the [beastify](https://github.com/mdn/webextensions-examples/tree/master/beastify) extension example to illustrate the debugging features relevant to popups. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
To debug popups, use the [Toolbox](#developer-tools-toolbox) **Debugger** in the [split console][split-console] view so you can view the **Console** below **Debugger**.
...
With the popup open, its JavaScript sources are listed in **Debugger**. In the **Debugger** you can set breakpoints, step through code, modify the extension's state, and do [everything else you'd expect to be able to do in a debugger][debugger-link]. Any messages logged by your code display in **Console**.

![popup in debugger screenshot](/assets/img/documentation/develop/popup_in_debugger.png)

To inspect the popup's HTML and CSS, use **Inspector** in the [split console][split-console] view so you can view the **Console** below **Inspector**. You can [review and modify the popup's HTML and CSS in **Inspector**][page-inspector-link], as you would with any webpage.

![popup in inspector screenshot](/assets/img/documentation/develop/popup_in_inspector.png)

If your extension has multiple HTML documents open, click the page select icon (![pages selector][page-selector]) to open the popup's document.
...
::: note
We use the [notify-link-clicks-i18n][i18n-link] extension example to illustrate the debugging features relevant to content scripts. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
If you've already activated the [split console][split-console] view, so you can view the **Console** below **Debugger**, the tools open in this mode. Otherwise, press `esc` to activate this mode.

![content script in debug](/assets/img/documentation/develop/content_script_in_debug.png)

Select your content scripts listed in **Sources**. You can set breakpoints, step through code, modify the extension's state, and do [everything else you'd expect to be able to do in a debugger][debugger-link]. Any messages logged by your code display in **Console**.
...
::: note
We use the [annotate-page][annotate-page] extension example to illustrate the debugging features relevant to sidebars. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
To debug a sidebar, use the [Toolbox](#developer-tools-toolbox) **Debugger** in the [split console][split-console] view so you can view the **Console** below **Debugger**.
...
To inspect the sidebar's HTML and CSS, use **Inspector** in the [split console][split-console] view so you can view the **Console** below **Inspector**. You can [review and modify the sidebar's HTML and CSS in **Inspector**][page-inspector-link], as you would with any webpage.

![sidebar script in debugger](/assets/img/documentation/develop/sidebar_page_in_inspector.png)

If your extension has multiple HTML documents open, click the page select icon (![pages selector][page-selector]) to open the sidebar's document.
...
::: note
We use the [annotate-page][annotate-page] extension example to illustrate the debugging features relevant to extension storage. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
::: note
We use the [devtools-panels](https://github.com/mdn/webextensions-examples/tree/master/devtools-panels) extension example to illustrate the debugging features relevant to developer tools pages and panels. This example is in the [webextensions-examples][web-ext-link] repository.
:::
...
To debug development tools pages and panels, use the [Toolbox](#developer-tools-toolbox) **Debugger** in the [split console][split-console] view so you can view the **Console** below **Debugger**.
...
You can [review and modify the custom development tools page HTML and CSS in **Inspector**][page-inspector-link], as you would with any webpage.
